...
Options String format: 'option1 [#]|option2 [#]|option3 [#]'

There must only be one space between the option and specified number.
Case matters.

Supported rules:
num # - minimum amount of numbers needed in password is #,
    ...
    for (i = 0; i < options.length; ++i) {
      ...
      if (command.length === 2) {
        ...
        if (isNaN(arg2)) return next(new Error('Bad argument: ' + arg2));
          if (arg1 === 'num') {
            if (!num(data.pass, arg2)) {
              res.error('Password requires at least ' + arg2 + ' number(s)');
              res.redirect('back');
            }
          } else if (arg1 === 'minlen') {
